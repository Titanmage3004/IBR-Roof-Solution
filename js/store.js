...
const PRODUCTS = [
  ...
  {
    id: 'p2',
    title: 'Upper Roof Support Bracket',
    price: 5.0,
    colors: ['black', 'white'],
    img: '',
    imgByColor: {}
...
(function ensureRandCurrencyAndPrices() {
  try {
    ...
    cur.priceOverrides['p1'] = 5.0; // Inner Rib Block
    cur.priceOverrides['p2'] = 5.0; // Upper Roof Support Bracket
    cur.priceOverrides['p3'] = 3.5; // Hurricane Clip
    localStorage.setItem(key, JSON.stringify(cur));
  } catch (e) {
    console.warn('ensureRandCurrencyAndPrices failed', e);
  }
})();
...
function renderProducts() {
  ...
  PRODUCTS.forEach((p) => {
    ...
    priceLabel.textContent = 'Price (R):';
    const priceMainRow = document.createElement('div');
    priceMainRow.className = 'price-main-row';
    const pricePill = document.createElement('div');
    pricePill.className = 'price-pill';
    pricePill.textContent = formatCurrency(getProductPrice(p.id));
    const perUnit = document.createElement('div');
    perUnit.className = 'price-per-unit';
    perUnit.textContent = 'per unit';
    priceMainRow.appendChild(pricePill);
    priceMainRow.appendChild(perUnit);
    const priceMin = document.createElement('div');
    priceMin.className = 'price-min';
    priceMin.textContent = 'Minimum order: 5 units';
    priceRow.appendChild(priceLabel);
    priceRow.appendChild(priceMainRow);
    ...
    colorLabel.textContent = 'Color Selection:';
    const swatches = document.createElement('div');
    swatches.className = 'swatches';
    swatches.setAttribute('data-id', p.id);
    p.colors.forEach((c) => {
      ...
      b.style.background = getColorCss(c);
    ...
    unitsLabel.className = 'units-label';
    unitsLabel.textContent = 'Units Required:';
    const qtyControls = document.createElement('div');
    qtyControls.className = 'qty-controls';
    qtyControls.setAttribute('data-id', p.id);
    const decBtn = document.createElement('button');
    decBtn.className = 'qty-button btn-decrease';
    decBtn.setAttribute('data-id', p.id);
    decBtn.setAttribute('aria-label', 'Decrease');
    decBtn.textContent = '−';
    const valDiv = document.createElement('div');
    valDiv.className = 'qty-value';
    valDiv.setAttribute('data-id', p.id);
    valDiv.textContent = '5';
    const incBtn = document.createElement('button');
    incBtn.className = 'qty-button btn-increase';
    incBtn.setAttribute('data-id', p.id);
    incBtn.setAttribute('aria-label', 'Increase');
    incBtn.textContent = '+';
    qtyControls.appendChild(decBtn);
    qtyControls.appendChild(valDiv);
    qtyControls.appendChild(incBtn);
    unitsRow.appendChild(unitsLabel);
    unitsRow.appendChild(qtyControls);

    left.appendChild(colorRow);
    left.appendChild(unitsRow);
  ...
  document.querySelectorAll('.btn-increase').forEach((b) =>
    b.addEventListener('click', (e) => {
      ...
      let v = parseInt(valEl.textContent || '5');
      v = normalizeQty(v + 5);
      valEl.textContent = v;
      card.dataset.qty = v;
    })
  ...
  document.querySelectorAll('.add-to-cart').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      if (id) {
        ...
        const card = document.querySelector(`[data-product-id="${id}"]`);
        const color = card
          ? card.dataset.currentColor
          : PRODUCTS.find((p) => p.id === id).colors[0];
...
function initModelViewerFallbacks() {
  ...
  const viewers = Array.from(
    document.querySelectorAll('#products model-viewer, .diy-kit model-viewer')
  );
  viewers.forEach((v) => {
    v.addEventListener('error', (ev) => {
      try {
        ...
        console.warn('fallback switch failed', e);
      }
    });
...
function getAdminSettings() {
  try {
    return JSON.parse(localStorage.getItem('site_admin') || '{}');
  } catch (e) {
    return {};
  }
}
...
function getProductPrice(id) {
  ...
  if (admin && admin.priceOverrides && admin.priceOverrides[id] != null)
    return Number(admin.priceOverrides[id]);
  return base;
}
...
function renderCartItems() {
  // optional cart modal element (not required for rendering logic)
  ...
  cart.forEach((it, idx) => {
    ...
    // product metadata is available if needed: PRODUCTS.find(p=>p.id===it.id)
    ...
    colorSwatch.style.background = getColorCss(it.color);
    colorDiv.appendChild(colorSwatch);
    const colorText = document.createTextNode(String(it.color));
    colorDiv.appendChild(colorText);
    meta.appendChild(titleDiv);
    meta.appendChild(colorDiv);
    ...
    qtyBox.style.cssText = 'display:flex;align-items:center;gap:0.5rem';
    const decBtn = document.createElement('button');
    decBtn.className = 'qty-decrease';
    decBtn.setAttribute('data-idx', String(idx));
    decBtn.setAttribute('aria-label', 'Decrease quantity');
    decBtn.textContent = '−';
    const qtyDisplay = document.createElement('div');
    qtyDisplay.className = 'qty-display';
    qtyDisplay.setAttribute('data-idx', String(idx));
    qtyDisplay.setAttribute('aria-live', 'polite');
    qtyDisplay.textContent = String(it.qty);
    const incBtn = document.createElement('button');
    incBtn.className = 'qty-increase';
    incBtn.setAttribute('data-idx', String(idx));
    ...
    removeWrap.style.cssText = 'display:flex;align-items:center;gap:0.5rem';
    const removeBtn = document.createElement('button');
    removeBtn.className = 'remove-item';
    removeBtn.setAttribute('data-idx', String(idx));
    removeBtn.setAttribute('aria-label', `Remove ${it.title}`);
    removeBtn.textContent = '🗑';
    removeWrap.appendChild(removeBtn);
    qtyControls.appendChild(qtyBox);
    qtyControls.appendChild(removeWrap);
    right.appendChild(qtyControls);
    const lineTotal = document.createElement('div');
    lineTotal.className = 'line-total text-sm';
    lineTotal.style.marginTop = '0.5rem';
    lineTotal.textContent = formatCurrency(currentPrice * it.qty);
  ...
  if (totalEl) {
    ...
    if (totalEl) {
      ...
      subDiv.style.fontSize = '0.95rem';
      subDiv.style.color = '#6b7280';
      subDiv.textContent = `Subtotal: ${formatCurrency(subtotal)}`;
      const shipDiv = document.createElement('div');
      shipDiv.style.fontSize = '0.95rem';
      shipDiv.style.color = '#6b7280';
      shipDiv.textContent = `Shipping: ${formatCurrency(shipping)}`;
      const grandDiv = document.createElement('div');
      grandDiv.style.fontWeight = '800';
      grandDiv.style.fontSize = '1.05rem';
      grandDiv.style.marginTop = '0.25rem';
      grandDiv.textContent = formatCurrency(grandTotal);
      totalEl.appendChild(subDiv);
      totalEl.appendChild(shipDiv);
      totalEl.appendChild(grandDiv);
    }
  }
  ...
  document.querySelectorAll('.qty-increase').forEach((btn) =>
    btn.addEventListener('click', () => {
      ...
      const cart = JSON.parse(localStorage.getItem('cart') || '[]');
      if (cart[idx]) {
        cart[idx].qty = normalizeQty(Number(cart[idx].qty || 0) + 5);
        localStorage.setItem('cart', JSON.stringify(cart));
        renderCartItems();
        window.app.updateCartCount();
      }
  ...
  document.querySelectorAll('.qty-decrease').forEach((btn) =>
    btn.addEventListener('click', () => {
      ...
      const cart = JSON.parse(localStorage.getItem('cart') || '[]');
      if (cart[idx]) {
        cart[idx].qty = Math.max(5, Number(cart[idx].qty || 5) - 5);
        localStorage.setItem('cart', JSON.stringify(cart));
        renderCartItems();
        window.app.updateCartCount();
      }
...
function initKitHandlers() {
  ...
  kit.querySelectorAll('.btn-increase[data-id="kit"]').forEach((b) =>
    b.addEventListener('click', () => {
      qtyValue.textContent = String(normalizeQty(parseInt(qtyValue.textContent || '5') + 5));
    })
  );
  kit.querySelectorAll('.btn-decrease[data-id="kit"]').forEach((b) =>
    b.addEventListener('click', () => {
      qtyValue.textContent = String(Math.max(5, parseInt(qtyValue.textContent || '5') - 5));
    })
  );
  ...
  if (kitAdd) {
    kitAdd.addEventListener('click', () => {
      ...
      if (existing) {
        existing.qty = normalizeQty(Number(existing.qty || 0) + qty);
      } else {
        cart.push({ id: kitId, title: kitTitle, price: kitPrice, color: selectedColor, qty: qty });
      }
      localStorage.setItem('cart', JSON.stringify(cart));
...
function adjustInvoiceActionsForMobile() {
  try {
    const isMobile =
      window.innerWidth <= 900 || /Mobi|Android|iPhone|iPad|Mobile/.test(navigator.userAgent || '');
    ...
    console.warn('adjustInvoiceActionsForMobile failed', e);
  }
}
...
if (printInvoiceBtn)
  printInvoiceBtn.addEventListener('click', () => {
    ...
    const doInlinePrint = () => {
      ...
      const cleanup = () => {
        try {
          const el = document.getElementById('printInvoiceContainer');
          if (el) el.remove();
        } catch (e) {
          void e;
        }
      ...
      try {
        ...
        const cssHref =
          Array.from(document.querySelectorAll('link[rel="stylesheet"]'))
            ...
            .find((h) => h && h.includes('styles.css')) || 'css/styles.css';
        const doc = newWin.document;
        doc.open();
        // build a minimal printable document using escaped text where appropriate
        const printHtml = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Invoice</title><link rel="stylesheet" href="${escapeHTML(
          cssHref
        )}"></head><body>`;
        ...
        console.warn('mobile print window failed', e);
        doInlinePrint();
        return;
      }
    }
  ...
  downloadInvoiceBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        const invoiceNumber =
          invoiceBody.dataset && invoiceBody.dataset.invoiceNumber
            ? invoiceBody.dataset.invoiceNumber
            : 'invoice';
...
function renderInvoice(cart) {
  ...
  let invoiceNumber =
    invoiceBody.dataset && invoiceBody.dataset.invoiceNumber
      ...
      : null;
  if (!invoiceNumber) {
    invoiceNumber = String(getNextInvoiceNumber());
    if (invoiceBody && invoiceBody.dataset) invoiceBody.dataset.invoiceNumber = invoiceNumber;
  }
  // Invoice rows are constructed via DOM APIs below; previous string-based
  // templates were removed to reduce XSS risk and improve maintainability.

  const subtotal = (cart || []).reduce(
    (s, it) => s + (it.price != null ? Number(it.price) : getProductPrice(it.id)) * it.qty,
    0
  );
  ...
  // WhatsApp link is constructed as a proper anchor element below; avoid inline HTML string.
  ...
  logo.src = 'assets/All Products Patented.svg';
  logo.alt = 'IBR Roof Solutions';
  ...
  orgName.style.fontWeight = '900';
  orgName.style.fontSize = '1.05rem';
  orgName.textContent = 'IBR Roof Solutions';
  const meta = document.createElement('div');
  meta.className = 'invoice-meta';
  meta.textContent = `Invoice #: ${invoiceNumber} • Date: ${new Date().toLocaleDateString()}`;
  orgWrap.appendChild(orgName);
  orgWrap.appendChild(meta);
  headerLeft.appendChild(logo);
  headerLeft.appendChild(orgWrap);
  const headerRight = document.createElement('div');
  headerRight.style.textAlign = 'right';
  const amtDue = document.createElement('div');
  amtDue.style.fontWeight = '800';
  amtDue.style.fontSize = '1.1rem';
  amtDue.textContent = formatCurrency(total);
  const amtLabel = document.createElement('div');
  amtLabel.className = 'invoice-meta';
  amtLabel.textContent = 'Amount Due';
  headerRight.appendChild(amtDue);
  headerRight.appendChild(amtLabel);
  ...
  table.setAttribute('aria-label', 'Invoice items');
  const thead = document.createElement('thead');
  const htr = document.createElement('tr');
  ['#', 'Item', 'Color', 'Qty', 'Unit', 'Total'].forEach((t) => {
  ...
  (cart || []).forEach((it, idx) => {
    ...
    const cols = [
      ...
      it.color,
      String(it.qty),
      formatCurrency(it.price != null ? Number(it.price) : getProductPrice(it.id)),
      formatCurrency((it.price != null ? Number(it.price) : getProductPrice(it.id)) * it.qty)
    ];
    cols.forEach((c) => {
  ...
  summaryWrap.className = 'invoice-summary';
  const summaryInner = document.createElement('div');
  summaryInner.style.textAlign = 'right';
  const subDiv = document.createElement('div');
  subDiv.style.fontWeight = '700';
  subDiv.textContent = `Subtotal: ${formatCurrency(subtotal)}`;
  const shipDiv = document.createElement('div');
  shipDiv.style.fontWeight = '700';
  shipDiv.textContent = `Shipping: ${formatCurrency(shipping)}`;
  const grandDiv = document.createElement('div');
  grandDiv.style.fontWeight = '900';
  grandDiv.style.marginTop = '0.25rem';
  grandDiv.textContent = `Total: ${formatCurrency(total)}`;
  summaryInner.appendChild(subDiv);
  summaryInner.appendChild(shipDiv);
  summaryInner.appendChild(grandDiv);
  summaryWrap.appendChild(summaryInner);
  invoiceBody.appendChild(summaryWrap);

  // bank details + QR placeholder
  const footerWrap = document.createElement('div');
  footerWrap.style.display = 'flex';
  footerWrap.style.gap = '1rem';
  footerWrap.style.marginTop = '1rem';
  ...
  bankTitle.style.fontWeight = '800';
  bankTitle.style.marginBottom = '0.25rem';
  bankTitle.textContent = 'Banking details (placeholder)';
  bankDetails.appendChild(bankTitle);
  [
    'Account name: IBR Roof Solutions',
    'Bank: Example Bank',
    'Branch code: 000000',
    ...
    d.textContent = t;
    bankDetails.appendChild(d);
  });
  const bankNote = document.createElement('div');
  bankNote.style.marginTop = '0.5rem';
  bankNote.style.fontSize = '0.95rem';
  bankNote.style.color = '#6b7280';
  bankNote.textContent = 'Use your invoice number as payment reference.';
  bankDetails.appendChild(bankNote);
  bankCol.appendChild(bankDetails);
  footerWrap.appendChild(bankCol);
  const qrCol = document.createElement('div');
  qrCol.style.width = '180px';
  qrCol.style.flex = '0 0 180px';
  const qr = document.createElement('div');
  qr.className = 'qr-code';
  qr.setAttribute('aria-hidden', 'true');
  qr.textContent = 'QR';
  const qrNote = document.createElement('div');
  qrNote.style.fontSize = '0.85rem';
  qrNote.style.color = '#6b7280';
  qrNote.style.marginTop = '0.5rem';
  qrNote.textContent = 'Scan to pay (placeholder)';
  qrCol.appendChild(qr);
  qrCol.appendChild(qrNote);
  footerWrap.appendChild(qrCol);
  invoiceBody.appendChild(footerWrap);

  // proof of payment
  const proofWrap = document.createElement('div');
  proofWrap.style.marginTop = '0.85rem';
  proofWrap.style.borderTop = '1px dashed rgba(0,0,0,0.06)';
  proofWrap.style.paddingTop = '0.85rem';
  const proofTitle = document.createElement('div');
  proofTitle.style.fontWeight = '800';
  proofTitle.style.marginBottom = '0.25rem';
  proofTitle.textContent = 'Send proof of payment';
  const proofText = document.createElement('div');
  proofText.style.color = '#1f2937';
  const mailA = document.createElement('a');
  mailA.href = `mailto:${contactEmail}?subject=Payment%20Proof%20Invoice%20${invoiceNumber}`;
  mailA.style.color = 'var(--brand)';
  mailA.style.fontWeight = '700';
  mailA.textContent = contactEmail;
  const waA = document.createElement('a');
  waA.href = `https://wa.me/${waDigits}?text=${waText}`;
  waA.setAttribute('target', '_blank');
  waA.setAttribute('rel', 'noopener');
  waA.textContent = whatsappRaw;
  proofText.appendChild(document.createTextNode('Email your proof to '));
  proofText.appendChild(mailA);
  proofText.appendChild(document.createTextNode(' or send via WhatsApp: '));
  proofText.appendChild(waA);
  const proofNote = document.createElement('div');
  proofNote.style.marginTop = '0.5rem';
  proofNote.style.fontSize = '0.9rem';
  proofNote.style.color = '#6b7280';
  proofNote.textContent =
    'Please include your invoice number in the message so we can match your payment.';
  proofWrap.appendChild(proofTitle);
  proofWrap.appendChild(proofText);
  proofWrap.appendChild(proofNote);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if (dl) {
      dl.setAttribute('data-desktop-label', dl.textContent || 'Download PNG');
      dl.setAttribute('data-mobile-label', 'Save');
    }
    adjustInvoiceActionsForMobile();
    window.addEventListener('resize', () => adjustInvoiceActionsForMobile());
  } catch (e) {
    console.warn('invoice init failed', e);
  }
});
